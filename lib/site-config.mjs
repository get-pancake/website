 ...
 *
 * Each origin reads a NEXT_PUBLIC_* variable and falls back to its default
 * below: pancake.ai for the site, the app and the analytics ingest since the
 * cutover, getpancake.ai for the hosts that have not moved yet. Setting a
 * variable back to its getpancake.ai value rolls one origin back. Next inlines
 * NEXT_PUBLIC_* values at BUILD time, and only for a literal
 * `process.env.NEXT_PUBLIC_…` read (never a computed key), so a new value
 * needs a redeploy. A malformed value fails the build instead of shipping a
 * broken link.
 *
...
/** Each origin when its variable is unset. */
export const DEFAULT_ORIGINS = Object.freeze({
  ...
  // Provider custom domains and the squads service move once they serve pancake.ai.
 ...
 */
export const PANCAKE_DOMAINS = Object.freeze(["pancake.ai", "getpancake.ai"]);
...
/** The website's canonical hostname, e.g. "pancake.ai". */
...
/**
 * A Google identity that belongs to one of the staff Workspace domains: a verified
 * email on an allowed domain AND an `hd` (hosted domain) claim on an allowed
 * domain. `hd` is present only for accounts managed by a Google Workspace, so
 * this refuses unmanaged ("consumer") Google accounts that a previous owner of a
 * domain may still hold with an address on it — the risk that arrives with
 * pancake.ai, a domain bought second-hand (PAN-1318).
 *
 * @param {{ email?: string, email_verified?: boolean | string, hd?: string }} claims
 * @param {readonly string[]} domains
 * @returns {boolean}
 */
export function isStaffGoogleIdentity(claims, domains) {
  ...
  return domains.includes(email.slice(at + 1)) && domains.includes(hostedDomain);
}
