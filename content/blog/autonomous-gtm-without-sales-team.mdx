...
New leads arrive each morning, each with the signal that picked it. You approve them in the app or from Slack. Pancake then runs the outreach from your own account on the professional social network: a profile visit, a like on a recent post, an invite with no note, then up to three messages. The first message makes no pitch. It asks one light question tied to the signal. Replies come to your own account, where a founder picks up the thread.
